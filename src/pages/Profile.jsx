    ...
    <div className="space-y-6">
      ...
      <div className="card p-6 flex items-center gap-6 animate-slide-up fill-both">
        <div className="relative group">
          ...
          <button className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-obsidian-600 border-2 border-obsidian-800 flex items-center justify-center text-ink-500 hover:text-ink-900 transition-colors">
            <Camera size={11} />
          </button>
        </div>
        <div className="flex-1">
          <h2 className="font-display font-bold text-xl text-ink-900">{user.firstName} {user.lastName}</h2>
        ...
        <div className="text-right hidden lg:block">
          ...
          <p className="font-display font-bold text-2xl text-neon-green">{fmt(user.walletBalance)}</p>
      ...
      {tab === 'profile' && (
        <div className="card p-6 animate-slide-up fill-both">
          <h3 className="font-display font-semibold text-base text-ink-900 mb-5">Personal Information</h3>
          <form onSubmit={handleProfileSave} className="space-y-4 max-w-lg">
            ...
            <FormGroup label="Email" hint="Changing email requires re-verification">
              <Input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} required />
            </FormGroup>
            <FormGroup label="Phone (optional)">
              <Input type="tel" placeholder="+1234567890" value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} />
            </FormGroup>
      ...
      {tab === 'security' && (
        <div className="space-y-4 animate-slide-up fill-both">
          <div className="card p-6">
            <h3 className="font-display font-semibold text-base text-ink-900 mb-1">Change Password</h3>
            <p className="text-sm text-ink-500 mb-5">Use a strong password with uppercase, lowercase, numbers and symbols.</p>
            <form onSubmit={handlePasswordSave} className="space-y-4 max-w-lg">
              <FormGroup label="Current Password">
                <Input type="password" placeholder="••••••••" value={passForm.current} onChange={e => setPassForm(f => ({ ...f, current: e.target.value }))} required />
              </FormGroup>
              <FormGroup label="New Password">
                <Input type="password" placeholder="Min 8 chars with special character" value={passForm.newPass} onChange={e => setPassForm(f => ({ ...f, newPass: e.target.value }))} required />
              </FormGroup>
          ...
          <div className="card p-6">
            <h3 className="font-display font-semibold text-base text-ink-900 mb-1">Two-Factor Authentication</h3>
            <p className="text-sm text-ink-500 mb-4">Every login requires a 6-digit code sent to your email.</p>
            <div className="flex items-center gap-3 p-4 rounded-xl bg-neon-green/5 border border-neon-green/20">
              ...
              <div>
                <p className="text-sm font-semibold text-neon-green">2FA is enabled</p>
                <p className="text-xs text-ink-500">Email OTP is sent on every sign-in</p>
              </div>
